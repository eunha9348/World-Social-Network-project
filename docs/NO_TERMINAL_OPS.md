...
**`hn-search`에 실제로 조사하려는 주제를 넣는 편이 낫다.**

다국어는 `mastodon`으로 확보한다. `instances`에 언어권별 호스트를 쉼표로 나열하고
`languages`에 유지할 코드를 적는다. 인스턴스가 공개 타임라인을 닫았으면 그 호스트만
건너뛰고 나머지는 계속한다(결과의 `unreachable` 참고).

**확보할 수 없는 출처**: 네이버·디시인사이드·클리앙 등 국내 대형 커뮤니티는 공개 API가 없고
크롤링은 약관 위반이다. Instagram·Facebook·LinkedIn도 대량 수집 권한이 없다.
한국어를 정당하게 늘리려면 한국어 Mastodon 인스턴스나 데이터 제공 계약이 필요하다.
`docs/COMMUNITY_CATALOG.md`의 97개는 후보 목록이지 연동된 출처가 아니다.
...
목표는 수집부터 검색까지 완전 무인화지만, **현재 호스팅에서는 마지막 한 단계가 막힌다.**

| 단계 | 무인화 | 비고 |
|---|---|---|
| 1. 공개 API에서 신규 글 수집 | ✅ | `collect_all.py`가 계획의 9개 그룹을 순회 |
| 2. 중복 제거·재개 | ✅ | `data/index.sqlite`와 커서를 Actions 캐시에 보관 |
| 3. Gemini 언어·감성·의견 분류 | ✅ | |
| 4. Qdrant 임베딩 저장 | ✅ | |
| 5. **VMAX DB 저장** | ❌ | **ChatGPT Sites가 서버 간 호출을 403으로 막는다** |
| 6. 사용자 자유 검색 | ✅ | 5번이 끝난 원문에 한해 |

5번은 코드 문제가 아니라 호스팅 제약이다(`3-2-1` 참고). 그래서 지금은
**1–4번이 매일 자동으로 돌고, 관리자가 파일 하나를 올리면 5번이 끝난다.**
완전 무인화는 서버 간 호출을 받는 호스팅으로 옮겨야 가능하다.
...
| 이름 | 값 | 뜻 |
|---|---|---|
| `SCHEDULE_ENABLED` | `true` | **이 값이 없으면 스케줄은 아무것도 하지 않는다** |
| `SCHEDULE_PER_GROUP` | `8` (기본) | 그룹당 수집 건수 |
| `SCHEDULE_MAX_DOCUMENTS` | `40` (기본) | 하루 색인 상한. **비용 상한이기도 하다** |

매일 03:00 UTC(정오 KST)에 돌며 9개 그룹을 각각 수집한다.
그룹마다 예산을 따로 주는 이유는, 하나의 공용 예산을 두면 가장 빨리 응답하는 출처가
다 가져가서 corpus가 다시 단일 언어로 쏠리기 때문이다.

**스케줄은 기본으로 꺼져 있다.** 감시자 없이 모델 비용이 나가는 동작이므로,
워크플로가 존재한다는 것이 동의는 아니다. `SCHEDULE_ENABLED`를 직접 켜야 시작한다.

### 관리자가 할 일

하루 한 번, Actions의 최신 실행에서 `upload-to-admin-ingest.jsonl`을 받아
`/admin/ingest`에 올린다. 파일에 수백·수천 건이 있어도 화면이 API 상한인 **20건씩 끊어** 보낸다.
페이지 상단에 현재 저장된 원문 수와 출처·언어 분포가 표시되므로, 업로드가 필요한지
추측하지 않고 확인할 수 있다.
