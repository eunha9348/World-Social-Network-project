...
// Relevance leads. Sorting by provenance first answered "who documented themselves best" instead
// of "what matches the query", which on a small corpus returns the whole corpus in author order.
// Provenance and the follower signal break ties, and no author may take more than two slots.
export function rankEvidence(posts:Post[],limit=10){const sorted=posts.map(p=>({...p,provenanceScore:provenanceScore(p)})).sort((a,b)=>(b.score||0)-(a.score||0)||(b.provenanceScore+followerSignal(b))-(a.provenanceScore+followerSignal(a)));const selected:Post[]=[];const rest:Post[]=[];const authorCount=new Map<string,number>();for(const p of sorted){const key=p.source+':'+(p.authorHandle||p.authorName||p.id);const count=authorCount.get(key)||0;if(count>=2){rest.push(p);continue}authorCount.set(key,count+1);selected.push(p)}return selected.slice(0,limit)}
